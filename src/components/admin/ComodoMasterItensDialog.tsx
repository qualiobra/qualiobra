...
import { useComodosMaxterItens } from "@/hooks/admin/useComodosMaxterItens";
...
interface ComodoMasterItensDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  comodo: ComodoMaster;
}

export const ComodoMasterItensDialog = ({
  open,
  onOpenChange,
  comodo,
}: ComodoMasterItensDialogProps) => {
  ...
  const {
    ...
    createComodoMasterItem,
    deleteComodoMasterItem,
    toggleObrigatorio,
    isCreating,
    isDeleting,
    isTogglingObrigatorio,
  } = useComodosMaxterItens(comodo.id);

  const { itens, isLoading: isLoadingAllItens } = useItensAdmin();

  // Atualizar ordem baseada nos itens existentes
  useEffect(() => {
    if (comodoMasterItens.length > 0) {
      const maxOrdem = Math.max(...comodoMasterItens.map(item => item.ordem));
      setOrdem(maxOrdem + 1);
    } else {
      setOrdem(1);
    }
  ...
  const itensDisponiveis = itens.filter(
    item => !comodoMasterItens.some(comodoItem => comodoItem.item_id === item.id) && item.ativo
  );
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Gerenciar Itens - {comodo.nome} (Padrão)
            </DialogTitle>
          </DialogHeader>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure os itens padrão que serão herdados por todos os cômodos baseados neste modelo.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="border rounded-lg p-4 space-y-4">
            <Label className="text-base font-semibold">Adicionar Item Padrão</Label>
          ...
          <div className="space-y-4">
            <Label className="text-base font-semibold">
              Itens Padrão ({comodoMasterItens.length})
            </Label>
            
            {comodoMasterItens.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground border rounded-lg">
                <Settings className="mx-auto h-12 w-12 mb-2 opacity-50" />
                <p>Nenhum item padrão configurado.</p>
